...
use DonlSync\Dataset\Builder\BuildRule\DCATDistributionBuildRule;
...
{
    /** @var int */
    public const MULTI_VALUED = 2;

    /** @var DefaultMapper */
    protected $default_values;

    /** @var ValueMapper[] */
    protected $value_mappings;

    /** @var BlacklistMapper[] */
    protected $blacklist_mappings;

    /** @var WhitelistMapper[] */
    protected $whitelist_mappings;

    /** @var IDCATEntityBuildRule[] */
    protected $custom_dataset_build_rules;

    /** @var IDCATEntityBuildRule[] */
    protected $custom_distribution_build_rules;
     ...
     *
     * @param string $catalog_name The name of the catalog from which the dataset is harvested
     * @param array  $source_data  The data from which to create a dataset
     *
    ...
    public function buildDataset(string $catalog_name, array $source_data): DatasetContainer
    {
        ...
        $this->buildURI('alternateIdentifier', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        ...
        $this->buildControlledVocabularyEntry('language', 'DONL:Language', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        ...
        $this->buildControlledVocabularyEntry('theme', 'Overheid:Taxonomiebeleidsagenda', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        $this->buildDateTime('modificationDate', $source_data, $dataset, $notices);
        $this->buildLiteral('keyword', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        $this->buildURI('landingPage', $source_data, $dataset, $notices);
        $this->buildSpatial('spatial', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        $this->buildTemporal('temporal', $source_data, $dataset, $notices);
        $this->buildURI('conformsTo', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        $this->buildURI('relatedResource', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        $this->buildURI('source', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        $this->buildURI('hasVersion', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        $this->buildURI('isVersionOf', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        $this->buildDateTime('releaseDate', $source_data, $dataset, $notices);
        $this->buildLiteral('version', $source_data, $dataset, $notices);
        $this->buildLiteral('versionNotes', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        ...
        $this->buildURI('documentation', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        $this->buildControlledVocabularyEntry('frequency', 'Overheid:Frequency', $source_data, $dataset, $notices);
        $this->buildURI('provenance', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        $this->buildURI('sample', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
        ...
        $this->buildDistribution('resources', $source_data, $dataset, $notices, DatasetBuilder::MULTI_VALUED);
     ...
     * @param string      $build_rule   The fully qualified classname of the buildrule to apply
                                       ...
                                       array &$notices, bool $multi_valued = false): void
    {
        if (array_key_exists($property, $this->custom_dataset_build_rules)) {
            ...
            $build_rule = new DCATDistributionBuildRule($property, 'Distribution');
            $build_rule->setDefaults($this->default_values);
            $build_rule->setValueMappers($this->value_mappings);
            $build_rule->setBlacklists($this->blacklist_mappings);
            $build_rule->setWhitelists($this->whitelist_mappings);
